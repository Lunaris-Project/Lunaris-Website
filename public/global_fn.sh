...
# scrDir is already set by the calling script
# If running from curl, scrDir will be the temporary directory
# If running locally, scrDir will be the directory containing this script

# If we're running from a pipe (curl), cloneDir should be the current directory
if [ ! -t 0 ]; then
    cloneDir="$(pwd)"
else
    # Otherwise, use the parent directory of scrDir
...
# Function to ensure a script is available when running from curl
ensure_script() {
    if [ ! -t 0 ] && [ ! -f "${scrDir}/$1" ] && [ -n "${base_url}" ]; then
        echo "Downloading missing script: $1..."
        curl -sL "${base_url}/$1" -o "${scrDir}/$1"
        chmod +x "${scrDir}/$1" 2>/dev/null || true
        return 0
    elif [ ! -f "${scrDir}/$1" ]; then
